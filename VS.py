...
def submit():
    ...
    #Calculate similarity and then sort. 
    if cmb.get() == 'tanimoto':
        dists = cdist(prop_target, prop_ref, metric='jaccard')
    else:
        dists = cdist(prop_target, prop_ref, metric=cmb.get())
    sim = 1 - normalization(dists)
    #np.savetxt('sim.csv', sim, fmt='%s', delimiter=',')
    idx = np.argsort(-sim, axis=1)
    k = int(neighbor.get())
    t = float(threshold.get())
    sim_sorted = np.take_along_axis(sim, idx, axis=1)
    f = open('result.csv', 'w')
    f.write('SMILES,Cluster,Index,Similarity\n')
    
    for i in range(len(mols1)):
        f.write(MolToSmiles(mols1[i]) + ',' + str(i + 1) + ',0,1' + '\n')
        #print(mols1[i].GetProp("DATABASE_ID"))
        idx_nearest = None
        line = sim_sorted[i]
        ...
        if nearest_mols == None:
            continue
        else:
            for j, m in enumerate(nearest_mols):
                f.write(m + ',' + str(i + 1) + ',' + str(j + 1) + ',' + str(line[j]) + '\n')
...
lb4.grid(row=3, column=0, columnspan=4, ipadx=85)

Label(lb4, text='Metrics:').grid(row=0, column=0, padx=5, pady=3)

cmb = Combobox(lb4, width=11)
cmb.grid(row=0, column=1, pady=3)
cmb['value'] = ('euclidean', 'hamming', 'minkowski', 'tanimoto')
cmb.current(0)

Label(lb4, text='Nearest Neighbors\nPer Molecule:', justify='center').grid(row=0, column=2, padx=5, pady=3)
neighbor = Entry(lb4, width=15)
neighbor.insert(0, 10)
neighbor.grid(row=0, column=3, pady=3)

Label(lb4, text='Threshold:').grid(row=0, column=4, padx=5, pady=3)
threshold = Entry(lb4, width=15)
threshold.insert(0, 0.8)
threshold.grid(row=0, column=5, pady=3)
